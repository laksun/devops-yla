...
def test_get_total_memory_in_gb(monkeypatch):
    ...
    class TestGetS3Export(unittest.TestCase):
        @patch("logmon_config_execution.get_service_client")
        def test_get_s3_export(self, mock_get_service_client):
            # Mock the return value of the exports method
            mock_exports = {
                "Exports": [
                    {"Name": "ApplicationLoggingBucket", "Value": "my-s3-bucket"}
                ]
            }
            mock_cf_client = mock_get_service_client.return_value
            mock_cf_client.exports.return_value = mock_exports

            # Call the function and check the result
            result = logmon_config_execution.get_s3_export("us-west-2", False)
